...
#for letter, pattern in CODEBOOK unpacks each tuple into two variables automatically.
...
for letter, pattern in CODEBOOK:
    ...
    if letter == 'Z':
        break
print(f"Comparisons needed to reach 'Z': {comparisons}")
...
#Letters with shortest pattern: ['E', 'T']
#   2. Compute the AVERAGE pattern length across all 26 letters.
#Letters with longest pattern: ['B', 'C', 'F', 'H', 'J', 'L', 'P', 'Q', 'V', 'X', 'Y', 'Z']
...
for number in range(len(CODEBOOK)):
    letter, pattern = CODEBOOK[number]
    print(f"{number + 1}. {letter}  {pattern}")
shortest_length = 4

for letter, pattern in CODEBOOK:
    if len(pattern) < shortest_length:
        shortest_length = len(pattern)

longest_length = 0

for letter, pattern in CODEBOOK:
    if len(pattern) > longest_length:
        longest_length = len(pattern)

shortest_letters = []

for letter, pattern in CODEBOOK:
    if len(pattern) == shortest_length:
        shortest_letters.append(letter)

longest_letters = []

for letter, pattern in CODEBOOK:
    if len(pattern) == longest_length:
        longest_letters.append(letter)
...
average_length = total_length / len(CODEBOOK)

print("Shortest pattern length:", shortest_length)
print("Letters with shortest pattern:", shortest_letters)
print("Longest pattern length:", longest_length)
print("Letters with longest pattern:", longest_letters)
print("Average pattern length:", average_length)
